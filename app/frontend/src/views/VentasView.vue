...
const handleCheckout = async ({ paymentMethod, shouldPrintTicket }: any) => {
    ...
    try {
        ...
        if (shouldPrintTicket) {
            generateTicketPDF(response.data, cart.value, companySettings.value, currency.value)
        }
...
<template>
    <div class="h-full flex flex-col lg:flex-row gap-2 lg:gap-4 animate-in fade-in duration-500 overflow-hidden relative" :class="uiStore.isZenMode ? 'p-2 lg:p-4 bg-background' : ''">
        ...
        <main class="flex-1 flex flex-col gap-4 overflow-hidden transition-all duration-300" :class="activeTab === 'cart' ? 'hidden lg:flex' : 'flex'">
            ...
            <!-- NEW: CATEGORIES AT TOP (3 COLUMNS) -->
            ...
            <div class="flex-1 flex flex-col min-w-0 space-y-4 overflow-hidden">
                <div class="flex items-center justify-between">
                    ...
                    <button @click="uiStore.toggleZenMode()" class="w-10 h-10 lg:w-12 lg:h-12 flex-shrink-0 flex items-center justify-center rounded-2xl bg-accent/30 text-foreground/50 hover:bg-primary/10 hover:text-primary transition-all group">
